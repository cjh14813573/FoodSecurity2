<template>
  <el-card class="box-card">
    ...
    <el-table :data="categoryList" height="100%" empty-text="无" highlight-current-row
      ...
      <!-- 新增对话框 -->
      <el-dialog title="新增大类" :append-to-body='true' :visible.sync="dialogNewVisible">
      ...
      <!-- 编辑对话框 -->
      <el-dialog title="编辑大类" :append-to-body='true' :visible.sync="dialogEditVisible">
  ...
  export default {
    ...
    methods: {
      handleCurrentChange(val) {
        if(val != null){
          this.currentRow = val;
          this.$emit('show-second', this.currentRow.id)
        }
      },
      updateCategoryList() {
        CategoryRequest.getTopList().then(response => {
          this.categoryList = response.data
          this.$emit('top-update')
        })
      },
      openPopover(index) {
      ...
      newAndCloseNewDialog() {
        ...
        CategoryRequest.newTop({
          name: this.newForm.name
        }).then(response => {
          Message({
            message: "成功新增食品大类  名称：" + response.data.name,
            type: 'success',
            duration: 2 * 1000
          })
      ...
      editAndCloseEditDialog() {
        ...
        CategoryRequest.editTop({
          id: this.editForm.id,
          name: this.editForm.name
        }).then(response => {
          Message({
            message: "食品大类名称已更改：" + response.data.name,
            type: 'success',
            duration: 2 * 1000
          })
          this.updateCategoryList()
