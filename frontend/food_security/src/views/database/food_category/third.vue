<template>
  <el-card class="box-card">
    ...
    <el-table :data="categoryList" height="100%" empty-text="无" highlight-current-row
      ...
      <!-- 新增对话框 -->
      <el-dialog title="新增次亚类" :append-to-body='true' :visible.sync="dialogNewVisible">
        <el-form ref="newForm" :rules="rules" :model="newForm" label-position="right" label-width="80px">
          <el-form-item label="所属亚类:">
            &nbsp;&nbsp;{{ parent.name }}
          </el-form-item>
      ...
      <el-dialog title="编辑亚类" :append-to-body='true' :visible.sync="dialogEditVisible">
        <el-form ref="editForm" :rules="rules" :model="editForm" label-position="right" label-width="80px">
          ...
          <el-form-item label="所属亚类" prop="second.id">
            <el-select v-model="editForm.second.id" placeholder="请选择所属亚类">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateCategoryList() {
        CategoryRequest.getThirdList(this.parent.id).then(response => {
          this.categoryList = response.data
          this.$emit('third-update')
        })
      },
      openPopover(index) {
      ...
      getParent() {
        CategoryRequest.getSecond(this.parent.id).then(response => {
      ...
      newAndCloseNewDialog() {
        ...
        CategoryRequest.newThird({
          name: this.newForm.name,
          parent: this.newForm.parent
        }).then(response => {
          Message({
            message: "成功新增食品次亚类  名称：" + response.data.name,
            type: 'success',
            duration: 2 * 1000
          })
          this.updateCategoryList()
      ...
      openEditDialog(row) {
        ...
        CategoryRequest.getSecond(this.parent.id).then(response => {
          ...
          this.editForm.second.id = row.parent
      ...
      editAndCloseEditDialog() {
        ...
        CategoryRequest.editThird({
          id: this.editForm.this.id,
          name: this.editForm.this.name,
          parent: this.editForm.second.id
        }).then(response => {
          Message({
            message: "食品次亚类已更改：" + response.data.name,
            type: 'success',
            duration: 2 * 1000
          })
          this.updateCategoryList()
