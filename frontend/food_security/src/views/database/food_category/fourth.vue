<template>
  <el-card class="box-card">
    ...
    <el-table :data="categoryList" height="100%" empty-text="无" highlight-current-row
      ...
      <!-- 新增对话框 -->
      <el-dialog title="新增细类" :append-to-body='true' :visible.sync="dialogNewVisible">
        <el-form ref="newForm" :rules="rules" :model="newForm" label-position="right" label-width="100px">
          <el-form-item label="所属次亚类:">
            &nbsp;&nbsp;{{ parent.name }}
          </el-form-item>
      ...
      <!-- 编辑对话框 -->
      <el-dialog title="编辑细类" :append-to-body='true' :visible.sync="dialogEditVisible">
        <el-form ref="editForm" :rules="rules" :model="editForm" label-position="right" label-width="100px">
          <el-form-item label="所属大类" prop="top.id">
            <el-select v-model="editForm.top.id" placeholder="请选择所属大类" @change="editFormOnTopChange()">
              <el-option v-for="top of topList" :key="top.id" :label="top.name" :value="top.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="所属亚类" prop="second.id">
            <el-select v-model="editForm.second.id" placeholder="请选择所属亚类" @change="editFormOnSecondChange()">
              <el-option v-for="second of secondList" :key="second.id" :label="second.name" :value="second.id">
              </el-option>
            </el-select>
          </el-form-item>
					<el-form-item label="所属次亚类" prop="third.id">
					  <el-select v-model="editForm.third.id" placeholder="请选择所属亚类">
					    <el-option v-for="third of thirdList" :key="third.id" :label="third.name" :value="third.id">
					    </el-option>
					  </el-select>
					</el-form-item>
  ...
  export default {
    ...
    methods: {
      handleCurrentChange(val) {
				/* if (val != null) {
          this.currentRow = val;
          this.$emit('show-fourth', this.currentRow.id)
        } */
      },
      updateCategoryList() {
        CategoryRequest.getFourthList(this.parent.id).then(response => {
          this.categoryList = response.data
          this.$emit('fourth-update')
        })
      },
      openPopover(index) {
      ...
      getParent() {
        CategoryRequest.getThird(this.parent.id).then(response => {
          this.parent.name = response.data.name
					this.parent.parent = response.data.parent
          this.editForm.second.id = response.data.parent
        })
      },

      openNewDialog() {
        this.getParent()
        this.newForm.parent = this.parent.id
      ...
      newAndCloseNewDialog() {
        ...
        CategoryRequest.newFourth({
          name: this.newForm.name,
          parent: this.newForm.parent
        }).then(response => {
          Message({
            message: "成功新增食品细类  名称：" + response.data.name,
            type: 'success',
            duration: 2 * 1000
          })
          this.updateCategoryList()
      ...
      updateSecondList() {
        CategoryRequest.getSecondList(this.editForm.top.id).then(response => {
          this.secondList = response.data
        })
      },
			updateThirdList() {
        CategoryRequest.getThirdList(this.editForm.second.id).then(response => {
      ...
      editFormOnTopChange() {
        ...
        this.editForm.second.id = null
        this.editForm.third.id = null
      },
			editFormOnSecondChange() {
				this.thirdList = []
      ...
      openEditDialog(row) {
        //先获取到级联父级类id才能更新列表
        CategoryRequest.getThird(this.parent.id).then(response => {
          this.editForm.third.name = response.data.name
          this.editForm.second.id = response.data.parent
					//获取top id以更新second列表
					CategoryRequest.getSecond(this.editForm.second.id).then(response => {
					  this.editForm.second.name = response.data.name
					  this.editForm.top.id = response.data.parent
					  ...
					  this.editForm.third.id = row.parent
      ...
      editAndCloseEditDialog() {
        ...
        CategoryRequest.editFourth({
          id: this.editForm.this.id,
          name: this.editForm.this.name,
          parent: this.editForm.third.id
        }).then(response => {
          Message({
            message: "食品细类已更改：" + response.data.name,
            type: 'success',
            duration: 2 * 1000
          })
          this.updateCategoryList()
    ...
    mounted() {
      this.parent.id = this.parentIdProp
      this.updateCategoryList()
      this.getParent()
    }
  }
